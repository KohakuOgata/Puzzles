...
(function () {
  ...
  function onClear() {
    ...
    if (best === null || used < best) {
      ...
      messageEl.textContent = "クリア！ " + used + " トライで成功 — 自己ベスト更新 🎉";
    } else {
      messageEl.textContent = "クリア！ " + used + " トライで成功（ベスト " + best + " トライ）";
    }
  }
  ...
  function gameOver() {
    ...
    messageEl.textContent = "トライを使い切ってしまいました…「新しい問題」で仕切り直しましょう。";
    paintBoard();
    paintTray();
    updateStats();
    updateButtons();
